...
exports.crearCuenta = async (peticion, respuesta) => {
  try {
    ...
    const nuevoUsuario = new Usuario({ nombre, email, password: hashedPassword, avatar });
    await nuevoUsuario.save();

    // Crear token JWT
    const token = jwt.sign({ id: nuevoUsuario._id }, process.env.CLAVE, { expiresIn: '1h' });
...
exports.iniciarSesion = async (peticion, respuesta) => {
  try {
    ...
    // Crear token JWT
    const token = jwt.sign({ id: usuario._id }, process.env.CLAVE, { expiresIn: '1h' });
